...
//		session.flush();
	...
	public static List<Match> findByName(String name) {
		return (List<Match>) session.createCriteria(Match.class).add(Restrictions.eq("name",name)).list();
	}
	
	
	@SuppressWarnings("unchecked")
	public static List<Match> findByTeams(Team team1, Team team2) {
		return (List<Match>) session.createCriteria(Match.class)
				.add(Restrictions.or(
						Restrictions.and(
								Restrictions.eq("hostTeam", team1), Restrictions.eq("guestTeam",team2)
						)
				, 
						Restrictions.and(
								Restrictions.eq("hostTeam", team2), Restrictions.eq("guestTeam",team1)
						)
				)).list();
	}
